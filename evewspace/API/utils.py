from evewspace.API.models import APICachedDocument
...
def store(host, path, params, doc, obj):
	...
	newDoc = APICachedDocument(host=host, params=params, path=path, xml=doc, cacheduntil=timestamp_to_datetime(obj.cachedUntil))
	newDoc.save()

def retrieve(host, path, params):
	"""Get an API document from our cache."""
	#Look for a valid API document in the cache, make sure it isn't stale and return it.
	try:
		document = APICachedDocument.objects.get(host=host, path=path, params=params)
		...
		#It shouldn't be possible for this to happen, so let's just return None and let it get sorted in store()
		return None
